...
export function Logo({ showWordmark = true, href = '/', className = '' }: LogoProps) {
  const wordmark = showWordmark ? (
    <span className="flex flex-col leading-tight">
      ...
      <span className="text-xl font-semibold text-slate-900">
        Analyzer <span className="text-indigo-600">Pro</span>
      </span>
    </span>
  ) : null
  ...
  return (
    <Link href={href} className={`group flex items-center gap-3 ${className}`}>
      <span className="relative grid h-11 w-11 place-items-center rounded-2xl bg-slate-950 text-white shadow-[0_12px_30px_-12px_rgba(79,70,229,0.6)] transition-transform group-hover:scale-[1.04]">
        <span className="absolute inset-0 rounded-2xl bg-gradient-to-br from-indigo-500 via-violet-600 to-purple-500 opacity-90" />
        <span className="absolute -bottom-1 inset-x-1 h-2 rounded-full bg-gradient-to-r from-cyan-400/50 to-indigo-500/60 blur-md" />
